...
from kafka_messages import settings
...
async def lifespan(app: FastAPI)-> AsyncGenerator[None, None]:
    ...
    task = asyncio.create_task(consume_messages('kafka-todo', 'broker:19092'))
...
app = FastAPI(lifespan=lifespan, title="Hello World API with DB", 
    ...
    servers=[
        {
            "url": "http://127.0.0.1:9000", # ADD NGROK URL Here Before Creating GPT Action
...
async def create_todo(todo: Todo, session: Annotated[Session, Depends(get_session)], 
        ...
        # Produce message
        await producer.send_and_wait("kafka-todo", todo_json)
